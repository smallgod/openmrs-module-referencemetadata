...
import org.openmrs.module.ohrireports.reporting.reports.OHRIReportsManager;
...
public class OhriReportsActivator extends BaseModuleActivator {
    ...
    public void started() {
	    ...
	    ReportManagerUtil.setupAllReports(OHRIReportsManager.class);
    }

	private void installConcepts() {
		GlobalProperty installedVersion = Context.getAdministrationService().getGlobalPropertyObject(OhriReportsConstants.INSTALLED_VERSION_GP);
        if (installedVersion == null) {
        	installedVersion = new GlobalProperty(OhriReportsConstants.INSTALLED_VERSION_GP, "0");
        }
        
        if (Integer.valueOf(installedVersion.getPropertyValue()) < OhriReportsConstants.METADATA_VERSION) {
            ...
            installedVersion.setPropertyValue(OhriReportsConstants.METADATA_VERSION.toString());
        }
    ...
    public void deployMetadataPackages(MetadataDeployService service) {
        MetadataBundle rolesAndPrivileges = Context.getRegisteredComponent("ohrireports.RolesAndPrivileges", MetadataBundle.class);
	    MetadataBundle orderFrequencies = Context.getRegisteredComponent("ohrireports.OrderFrequencies", MetadataBundle.class);
    ...
    public void setupOpenmrsId(AdministrationService administrationService, PatientService patientService, IdentifierSourceService identifierSourceService, MetadataMappingService metadataMappingService) {
        ...
        PatientIdentifierType openmrsIdType = patientService.getPatientIdentifierTypeByUuid(OhriReportsConstants.OPENMRS_ID_UUID);
        if (openmrsIdType == null) {
            openmrsIdType = new PatientIdentifierType();
            openmrsIdType.setName(OhriReportsConstants.OPENMRS_ID_NAME);
            openmrsIdType.setDescription(OhriReportsConstants.OPENMRS_ID_DESCRIPION);
            ...
            openmrsIdType.setUuid(OhriReportsConstants.OPENMRS_ID_UUID);
            patientService.savePatientIdentifierType(openmrsIdType);
        }

        SequentialIdentifierGenerator openmrsIdGenerator = (SequentialIdentifierGenerator) identifierSourceService.getIdentifierSourceByUuid(OhriReportsConstants.OPENMRS_ID_GENERATOR_UUID);
        if (openmrsIdGenerator == null) {
            ...
            openmrsIdGenerator.setName(OhriReportsConstants.OPENMRS_ID_GENERATOR_NAME);
            openmrsIdGenerator.setUuid(OhriReportsConstants.OPENMRS_ID_GENERATOR_UUID);
