...
package org.openmrs.module.ohrireports.reports;
...
public abstract class ReportManagerTest extends BaseModuleContextSensitiveTest {
	...
	public abstract void verifyData(ReportData data);
	...
	@Ignore //TODO: Ignored temporarily till Test is corrected for Patient
	@Test
	...
	public <T> Matcher<DataSetRow> hasData(final String expectedColName, final T expectedColValue) {
		return new FeatureMatcher<DataSetRow, T>(is(expectedColValue), expectedColName, expectedColName) {
			@Override
			protected T featureValueOf(DataSetRow actual) {
				return (T) actual.getColumnValue(expectedColName);
			}
		};
	}
